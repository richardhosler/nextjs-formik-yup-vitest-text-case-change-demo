...
describe("lower case tests", { concurrent: true }, () => {
  describe("using toLowerCase", { concurrent: true }, () => {
    test.for(testData.unstripped)("%s", ([input, expected]) => {
      expect(toLowerCase({ input })).toBe(expected);
    });
  });
  describe("using toLowerCase with alphaNumeric", { concurrent: true }, () => {
    test.for(testData.alphaNumeric)("%s", ([input, expected]) => {
      expect(toLowerCase({ input, options: { alphaNumeric: true } })).toBe(
        expected
      );
    });
  });
  describe("using toLowerCase with replace '_'", { concurrent: true }, () => {
    test.for(testData.replaced)("%s", ([input, expected]) => {
      expect(
        toLowerCase({ input, options: { alphaNumeric: true, replace: "_" } })
      ).toBe(expected);
    });
  });
  describe(
    ...
    () => {
      test.fails.for(testData.unstripped.slice(1, 3))(
        "%s",
        ([input, expected]) => {
          expect(changeCase.noCase(input)).toBe(expected);
        }
      );
    }
  );
