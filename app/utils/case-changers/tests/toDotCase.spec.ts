...
describe("dot.case tests", { concurrent: true }, () => {
  describe("using toDotCase", { concurrent: true }, () => {
    test.for(testData.unstripped)("%s", ([input, expected]) => {
      expect(toDotCase({ input })).toBe(expected);
    });
  });
  describe("using toDotCase with alphaNumeric", { concurrent: true }, () => {
    test.for(testData.alphaNumeric)("%s", ([input, expected]) => {
      expect(toDotCase({ input, options: { alphaNumeric: true } })).toBe(
        expected
      );
    });
  });
  describe("using toDotCase with replace '_'", { concurrent: true }, () => {
    test.for(testData.replaced)("%s", ([input, expected]) => {
      expect(
        toDotCase({ input, options: { alphaNumeric: true, replace: "_" } })
      ).toBe(expected);
    });
  });
  describe(
    ...
    () => {
      test.fails.for(testData.unstripped.slice(1, 3))(
        "%s",
        ([input, expected]) => {
          expect(changeCase.dotCase(input)).toBe(expected);
        }
      );
    }
  );
